...
"%JAVA_EXE%" -jar "%APPLICATION%" --json "%JSON_DIR%" --csdl "%CSDL_DIR%" --doc "%DOC_DIR%" --oem "%OEM_PREFIXES%" --filter "%FILTER_FILE%" --operations "%OPERATIONS_FILE%" --literals "%LITERALS_FILE%" --generatecode "%CODE_DIR%" --generateimpl --mapping "%MAPPING_FILE%" 2>&1
